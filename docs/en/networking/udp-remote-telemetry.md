...
>
> This page reproduces the original text verbatim (including original spelling); only formatting of code blocks has been restored.

::: warning Not covered by the original
How to enable remote telemetry in the game (the relevant configuration) is not covered by the original document and is not discussed here.
:::

This document helps you to set up UDP socket comunication between your application and Assetto Corsa. All the code samples are made using [codepad.org](http://codepad.org/).

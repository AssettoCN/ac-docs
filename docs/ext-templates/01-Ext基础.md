...
CSP EXT config本质是一堆ini文件，语法遵循CSP [inipp语法](https://lcn2yv4fj75n.feishu.cn/wiki/GCIEwCDS1iMQoAkgfE0cMhlKnvf)（一种专为csp设计的ini扩展语法）。

对于每个INI文件其整体结构主要由若干个`[SECTION]`块组成，而对于每个`SECTION`块来说主要由KEY和VALUE组成。  
在本章节你暂不需要了解如何编写EXT，只需要了解其大致框架即可，关于每个ext模板具体使用方式在具体章节会有介绍，如果想要了解详细完整inipp语法请参阅 [inipp语法](https://lcn2yv4fj75n.feishu.cn/wiki/GCIEwCDS1iMQoAkgfE0cMhlKnvf)章节
...
::: tip 🦄
对于EXT来说，写一个section， 就可以给我们的mod增加一个“效果”
:::

section的抽象结构只包含section名称，key和value
...
```text
[SECTION_...] ;省略号为自动索引也可以人工索引
KEY = VALUE ;KEY代表参数，VALUE代表值
```
...
以上例子就是一个反射特效的section，而它带有一个glass_filter参数，具体我们想要生效的值为 material:Taillight
...
::: tip 🏕️
让我们用一个官方的实际ext config 带你了解一个完整的配置文件都是如何工作的
:::

CSP中的可作用功能非常广泛，下面有一个csp的官方例子，里面的效果都会在后面的常用模板中提到，在这儿主要看一下，找找感觉即可

```ini
;在ini文件中都用;表示注释
;所有[]包裹的叫做section，我们称之为节
[INCLUDE] ;导入节,主要用于导入常用的预设好的ext配置
INCLUDE = common/no_popup_lights.ini

[EXTRA_FX] ;提供额外效果的节
SKIP_GBUFFER = INT_Displa_VEtro

[REFLECTIONS_FX] ;提供反射效果
GLASS_FILTER  =  material:Taillight
...
[SHADER_REPLACEMENT_...] ；重要！ 核心功能 shader替换
MATERIALS = Chrome ；生效材质
PROP_... = extExtraSharpLocalReflections, -0.001 ;shader参数替换

[SHADER_REPLACEMENT_...]
MATERIALS = Tyre_Tread ;生效材质
PROP_... = ksAmbient, 0.1 ;shader参数替换
PROP_... = ksDiffuse, 0.1
PROP_... = fresnelEXP, 5
PROP_... = fresnelMaxLevel, 0.1
PROP_... = fresnelC, 0

[INCLUDE: common/custom_emissive.ini] ;自定义发光特效 重要！
[CustomEmissive]
...
[SHAKING_EXHAUST_...]
MESHES = g_BODY_EXT_SUB5, g_BODY_EXT_SUB6@A
